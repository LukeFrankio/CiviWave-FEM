 ...
 *
 * this header crowns the Phase 8 milestone with a fully documented, matrix-free conjugate-gradient core. it
 * wires together packed GPU buffers, Rayleigh damping coefficients, and Newmark magic to deliver the exact
 * operator that the roadmap calls for. matrix-free apply, FP64-powered reductions, block-jacobi
 * preconditioning, warm-start awareness, and excessive doc comments? yeah, all of that lives here uwu ✨
 *
 * design goals:
 ...
 *
 * this module feeds both the CPU reference path and the upcoming Slang/Vulkan kernels. the tests compare
 * outputs against the dense CPU solver to guarantee numerical alignment before we unleash the GPU
 * implementation.
 *
 * @author LukeFrankio
 * @date 2025-11-12
 ...
 *
 * ✨ PURE FUNCTION ✨ metadata container — no ownership, just spans. everything here points into packed
 * buffers that were produced during Phase 7. the same structure feeds both CPU validation and Vulkan
 * descriptors. stick to the documented invariants and life stays chill.
 *
 * invariants:
...
/**
 * @brief reusable buffers for matrix-free apply + block-jacobi computation (no per-iteration allocations
 * allowed)
 */
 ...
 *
 * ✨ PURE FUNCTION ✨ — reads inputs, writes @p output, and bounces. no global state, no hidden allocations
 * (workspace owns the scratch). Dirichlet rows become identity, Dirichlet columns vanish thanks to sanitized
 * input.
 *
 ...
 * provided spans in-place, tracks telemetry, and performs iterative refinement with side effects confined to
 * @p vectors and @p workspace. external state remains untouched so tests can compare inputs/outputs cleanly.
 *
 ...
 *
 * @warning relative tolerance uses rhs_norm fallback of 1.0 when rhs is near-zero to avoid division-by-zero
 * crimes
