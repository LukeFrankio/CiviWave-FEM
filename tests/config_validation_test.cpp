...
auto make_invalid_cases() -> std::vector<InvalidConfigCase>
{
...

    {
        ...
        cases.push_back({
            "NegativeYoungsModulus",
            opts,
            "material.E must be > 0",
            {"materials", "[0]", "E"},
            nullptr
        });
    }
...

    {
        ConfigBuilderOptions opts{};
        opts.materials = {
            MaterialSpec{"duplicate", 3.0e10, 0.2, 2500.0},
...

    {
        ...
        cases.push_back({
            "NegativeSolverTolerance",
            opts,
            "solver tolerances must be > 0",
            {"solver"},
            nullptr
        });
    }

    {
        ...
        cases.push_back({
            "MissingPrecisionSection",
            opts,
            "missing precision map",
            {"precision"},
            nullptr
        });
    }
    ...
    {
        ConfigBuilderOptions opts{};
        opts.dirichlet_fixes = {DirichletSpec{"FIXED_BASE", {false, false, false}, {std::nullopt, std::nullopt, std::nullopt}}};
        cases.push_back({
            "EmptyDirichletDof",
            opts,
...
INSTANTIATE_TEST_SUITE_P(
    ExhaustiveInvalidConfigs,
    ConfigInvalidTest,
    ::testing::ValuesIn(make_invalid_cases()),
    [](const ::testing::TestParamInfo<InvalidConfigCase>& test_info) { return test_info.param.name; }
);
