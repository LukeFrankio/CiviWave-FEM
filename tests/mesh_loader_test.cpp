...
[[nodiscard]] auto tet_block_for_nodes(std::initializer_list<int> node_ids, std::uint32_t element_type = 4U)
    -> std::string
{
...
TEST(MeshLoader, LoadsCantileverFixtureAndCreatesPhysicalLookup)
{
    ...
    EXPECT_EQ(static_cast<std::uint32_t>(element.geometry),
              static_cast<std::uint32_t>(cwf::mesh::ElementGeometry::Tetrahedron4));
