...
void append_buffer(std::vector<LogicalBuffer> &buffers, std::string_view name,
                   std::span<const std::byte> bytes, std::size_t alignment)
{
...
auto build_logical_buffers(const mesh::pack::PackingResult                       &packing,
                           ...
                           PreparedGpuBuffers &prepared, const std::size_t alignment)
    -> std::vector<LogicalBuffer>
{
    ...
    append_buffer(buffers, "elements.gradients", as_bytes(make_span(packing.buffers.elements.gradients)),
                  effective_alignment);
    append_buffer(buffers, "elements.volume", as_bytes(make_span(packing.buffers.elements.volume)),
                  effective_alignment);
    ...
    append_buffer(buffers, "nodes.bc_mask", as_bytes(make_span(packing.buffers.nodes.bc_mask)),
                  effective_alignment);
    ...
    append_buffer(buffers, "adjacency.offsets", as_bytes(make_span(packing.buffers.adjacency.offsets)),
                  effective_alignment);
    append_buffer(buffers, "adjacency.indices",
                  as_bytes(make_span(packing.buffers.adjacency.element_indices)), effective_alignment);
    append_buffer(buffers, "adjacency.local_indices", as_bytes(make_span(prepared.adjacency_local_indices)),
                  effective_alignment);

    append_buffer(buffers, "solver.vector.p", as_bytes(make_span(packing.buffers.solver.p)),
                  effective_alignment);
    ...
    append_buffer(buffers, "solver.vector.z", as_bytes(make_span(packing.buffers.solver.z)),
                  effective_alignment);
    append_buffer(buffers, "solver.vector.x", as_bytes(make_span(packing.buffers.solver.x)),
                  effective_alignment);
    append_buffer(buffers, "solver.partials", as_bytes(make_span(packing.buffers.solver.partials)),
                  effective_alignment);
