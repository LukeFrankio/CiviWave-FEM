    ...
    -> std::expected<std::tuple<VkInstance, VkDebugUtilsMessengerEXT>, VulkanError>
{
    ...
    for (const auto &ext_name : required_instance_extensions)
    {
        ...
        {
            return std::unexpected(make_error(std::format("missing required instance extension {}", ext_name),
                                              {"vkCreateInstance"}));
        }
    ...
    if (info.enable_validation)
    {
        ...
        messenger_info.pfnUserCallback =
    ...
    -> std::expected<DeviceSelection, VulkanError>
{
    ...
    if (physical_count == 0U)
    {
        return std::unexpected(
            make_error("vkEnumeratePhysicalDevices returned zero devices", {"vkEnumeratePhysicalDevices"}));
    }
    ...
    for (std::uint32_t idx = 0U; idx < physical_devices.size(); ++idx)
    {
        ...
        descriptor_support.descriptor_buffer_push_descriptors =
            descriptor_buffer.descriptorBufferPushDescriptors == VK_TRUE;
        descriptor_support.descriptor_indexing =
        ...
        descriptor_support.max_descriptor_buffer_bindings =
            static_cast<std::uint32_t>(descriptor_props.maxDescriptorBufferBindings);
        descriptor_support.max_resource_descriptor_buffer_bindings =
            static_cast<std::uint32_t>(descriptor_props.maxResourceDescriptorBufferBindings);
        descriptor_support.max_sampler_descriptor_buffer_bindings =
            static_cast<std::uint32_t>(descriptor_props.maxSamplerDescriptorBufferBindings);
        descriptor_support.descriptor_buffer_address_space_size =
            descriptor_props.descriptorBufferAddressSpaceSize;
        descriptor_support.descriptor_buffer_offset_alignment =
    ...
    {
        return std::unexpected(
            make_error("no physical device satisfies Phase 5 requirements", {"device enumeration"}));
    }
    ...
    descriptor_buffer_features.descriptorBuffer =
        chosen.descriptor_support.descriptor_buffer ? VK_TRUE : VK_FALSE;
    descriptor_buffer_features.descriptorBufferPushDescriptors =
        chosen.descriptor_support.descriptor_buffer_push_descriptors ? VK_TRUE : VK_FALSE;
    ...
    device_info.ppEnabledExtensionNames =
        chosen.enabled_extensions.empty() ? nullptr : chosen.enabled_extensions.data();
    ...
    -> std::expected<StagingRing, VulkanError>
{
    ...
    {
        return std::unexpected(
            make_error("vmaCreateBuffer for staging ring failed", result, {"staging buffer"}));
    }

    auto ring = StagingRing{.allocation = allocation,
                            .buffer     = buffer,
    ...
    {
        return std::unexpected(
            make_error("staging buffer mapping returned null pointer", {"staging buffer"}));
    }
...
auto VulkanContext::create(const ContextCreateInfo &info) -> std::expected<VulkanContext, VulkanError>
{
    ...
    context.set_object_name(reinterpret_cast<std::uint64_t>(context.queue_info_.queue), VK_OBJECT_TYPE_QUEUE,
                            "cwf/compute-queue");
    context.set_object_name(reinterpret_cast<std::uint64_t>(context.staging_ring_.buffer),
                            VK_OBJECT_TYPE_BUFFER, "cwf/staging-ring");

    std::println(
        "[cwf::gpu] selected device: {} (vendor 0x{:04x}, type {}, api {} / driver {})",
        context.summary_.name, context.summary_.vendor_id, device_type_to_string(context.summary_.type),
        version_to_string(context.summary_.api_version), version_to_string(context.summary_.driver_version));
    ...
    std::println("[cwf::gpu] descriptor buffer: {}, indexing: {}, max bindings (descriptor/resource/sampler) "
                 "= {}/{}/{}, alignment {} bytes, address space {} bytes",
    ...
    -> std::expected<TimelineSemaphore, VulkanError>
{
    ...
    {
        return std::unexpected(
            make_error("vkCreateSemaphore (timeline) failed", result, {"timeline semaphore"}));
    }
...
void VulkanContext::push_debug_label(VkCommandBuffer cmd, std::string_view name,
                                     std::array<float, 4U> color) const
{
...
auto VulkanContext::make_memory_barrier(VkPipelineStageFlags2 src_usage, VkPipelineStageFlags2 dst_usage,
                                        VkAccessFlags2 src_access, VkAccessFlags2 dst_access) noexcept
    -> VkMemoryBarrier2
{
