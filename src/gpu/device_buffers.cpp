...
[[nodiscard]] auto create_command_pool(const VulkanContext &context)
    -> std::expected<VkCommandPool, VulkanError>
{
...
[[nodiscard]] auto allocate_command_buffer(VkDevice device, VkCommandPool pool)
    -> std::expected<VkCommandBuffer, VulkanError>
{
...
[[nodiscard]] auto submit_and_wait(const VulkanContext &context, VkCommandBuffer cmd)
    ...
    -> std::expected<DeviceBufferArena, VulkanError>
{
    ...
    for (const auto &segment : layout.segments)
    {
        if (segments_by_name.contains(segment.name))
        {
            return std::unexpected(make_error(
                "logical buffer spans multiple device buffers (sharding not yet supported)", {segment.name}));
        }
    ...
    for (std::size_t i = 0; i < layout.device_buffer_sizes.size(); ++i)
    {
        ...
        const VkResult result =
            vmaCreateBuffer(context.allocator(), &buffer_info, &alloc_info, &buffer, &allocation, nullptr);
        if (result != VK_SUCCESS)
        {
            arena.destroy();
    ...
    if (!schedule_expected)
    {
        arena.destroy();
        return std::unexpected(
            make_error(schedule_expected.error().message, schedule_expected.error().context));
    }
    ...
    const auto upload_status =
        upload_schedule(context, cmd, schedule_expected.value(), arena.device_buffers_);
    vkDestroyCommandPool(context.device(), command_pool, nullptr);
    if (!upload_status)
    {
        arena.destroy();
...
DeviceBufferArena::DeviceBufferArena(DeviceBufferArena &&other) noexcept
    : context_(other.context_), device_buffers_(std::move(other.device_buffers_)),
