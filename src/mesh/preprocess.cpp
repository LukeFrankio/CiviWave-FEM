...
[[nodiscard]] auto check_duplicate_nodes(const mesh::Mesh& mesh)
...
[[nodiscard]] auto check_duplicate_elements(const mesh::Mesh& mesh)
    -> std::expected<void, PreprocessError>
{
    ...
    for (std::size_t i = 0; i < mesh.elements.size(); ++i) {
        ...
        const auto node_count = static_cast<std::size_t>(
            elem.geometry == ElementGeometry::Tetrahedron4 ? 4U : 8U);
        ...
        if (indices.size() > 1) {
            for (std::size_t i = 0; i < indices.size(); ++i) {
                for (std::size_t j = i + 1; j < indices.size(); ++j) {
                    ...
                    if (elem_i.geometry != elem_j.geometry) continue;
                    
                    const auto count = static_cast<std::size_t>(
                        elem_i.geometry == ElementGeometry::Tetrahedron4 ? 4U : 8U);
                    ...
                    if (std::equal(sorted_i.begin(), sorted_i.begin() + static_cast<std::ptrdiff_t>(count),
                                  sorted_j.begin())) {
                        return std::unexpected(PreprocessError{
                            std::format("duplicate elements detected: element {} and element {} have same connectivity",
                                       indices[i], indices[j]),
                            {"mesh", "elements"}});
                    }
                }
...
[[nodiscard]] auto compute_tet_gradients(const std::array<Vec3, 4>& positions,
                                         double volume6)
    -> std::array<Vec3, 4>
{
    ...
    return {
        scale_vec(common::cross(subtract(p2, p1), subtract(p3, p1)), inv6),
...
auto run(const mesh::Mesh& mesh, const config::Config& cfg)
    -> std::expected<Outputs, PreprocessError>
{
    if (mesh.nodes.empty()) {
